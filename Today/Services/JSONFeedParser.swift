...
/// Parser for JSON Feed format (https://www.jsonfeed.org)
/// Inherits from NSObject for proper memory management with String/NSString bridging
class JSONFeedParser: NSObject {
    ...
    private func parseItemWithId(_ item: [String: Any], id: String) -> RSSParser.ParsedArticle {
        ...
        if let html = contentHtml, !html.isEmpty {
            processedContentEncoded = html.decodeHTMLEntities().texturize()
        }
        ...
        return RSSParser.ParsedArticle(
            title: title.decodeHTMLEntities().texturize(),
            link: link,
            description: description != nil ? description!.decodeHTMLEntities().texturize() : nil,
